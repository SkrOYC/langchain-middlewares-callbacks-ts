...
import type { createResponseBodySchema } from "../../../src/contract/generated/kubb/zod/createResponseBodySchema.ts";
import { responseResourceSchema } from "../../../src/contract/generated/kubb/zod/responseResourceSchema.ts";
...
const streamingSchema: ResponseValidator = (_, context) => {
  if (!context.streaming || !context.sseResult) {
    return [];
  }
...
const createResult = (
  result: Omit<TestResult, "streamEvents"> & { streamEvents?: number | undefined }
): TestResult => {
  if (result.streamEvents === undefined) {
    const { streamEvents: _streamEvents, ...withoutStreamEvents } = result;
    return withoutStreamEvents;
  }
...
async function runTest(
  ...
  try {
    ...
    const context: ValidatorContext =
      sseResult === undefined ? { streaming } : { streaming, sseResult };
    const errors = template.validators.flatMap((validator) =>
      validator(parseResult.data, context)
    );
