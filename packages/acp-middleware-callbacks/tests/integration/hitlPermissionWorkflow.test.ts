...
describe("HITL Permission Workflow Integration", () => {
		...
		test("agent pauses for permission, resumes with approval, executes tool", async () => {
			...
			const initialState = {
				messages: [
					{
						_getType: () => "human",
						content: "Write hello to file.txt and read it",
					},
					{
						_getType: () => "ai",
			...
			const runtime = {
				...
				interrupt: mock(async (hitlRequest: any) => {
					...
					expect(hitlRequest.reviewConfigs[0].allowedDecisions).toContain(
						"approve",
					);
			...
			const pendingUpdate = sessionUpdateCalls.find(
				(c: any) =>
					c.update.toolCallId === "tc-1" && c.update.status === "pending",
			);
						...
						tool_calls: [
							{
								id: "tc-1",
								name: "write_file",
								args: { path: "old.txt", content: "Original" },
							},
						],
					},
				],
	...
	describe("complete workflow with reject decision", () => {
		test("agent pauses, user rejects, execution jumps back to model", async () => {
			...
			expect(toolMessage.content[0].content.text).toBe(
			...
			const initialState = {
				messages: [
					{ _getType: () => "human", content: "Run dangerous tool" },
					{
						_getType: () => "ai",
						content: "I'll help you",
						tool_calls: [{ id: "tc-1", name: "dangerous_tool", args: {} }],
					},
				],
			};
						...
						tool_calls: [
							{
								id: "tc-1",
								name: "file_operation",
								args: { operation: "complex", data: { nested: true } },
							},
						],
					},
				],
			...
			const initialState = {
				messages: [
					{ _getType: () => "human", content: "Hello" },
					{
						_getType: () => "ai",
						content: "I'll help you",
						tool_calls: [{ id: "tc-1", name: "test_tool", args: {} }],
					},
				],
			};
	...
	describe("Command resume pattern demonstration", () => {
		test("demonstrates complete interrupt/resume workflow with Command pattern", async () => {
			...
			const runtime = {
				config: {
					configurable: { thread_id: "thread-1", session_id: "session-1" },
				},
				context: {},
				interrupt: mock(async (hitlRequest: any) => {
			...
			expect(extNotificationFn.mock.calls[0][0]).toBe(
		...
		test("demonstrates state checkpoint verification after interrupt", async () => {
			...
			const checkpointState = {
				messages: [
					...
					{
						_getType: () => "human",
						content: "Step 2: Perform dangerous operation",
					},
					{
						_getType: () => "ai",
						...
						tool_calls: [
							{
								...
								args: { target: "production", force: true },
							},
						],
					},
				],
			...
			expect(capturedInterruptRequest.actionRequests[0].name).toBe(
				"dangerous_operation",
			);
