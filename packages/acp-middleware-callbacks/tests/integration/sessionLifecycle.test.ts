...
describe("Full Session Lifecycle Integration", () => {
  describe("newSession → prompt → stream events flow", () => {
    test("complete session lifecycle with middleware and callbacks", async () => {
      ...
      const sessionMiddleware = createACPSessionMiddleware({
        ...
        onNewSession: mock((sessionId, state) => {
          lifecycleEvents.push({ type: "newSession", data: { sessionId, state } });
        }),
      ...
      const modelState = { ...initialState, messages: [{ role: "user", content: "Hello" }] };
      ...
      const finalState = {
        ...modelState,
        messages: [...modelState.messages, { role: "assistant", content: "I can help you!" }]
      };
    ...
    test("session with tool call lifecycle", async () => {
      ...
      const beforeAgent = sessionMiddleware.beforeAgent as any;
      await beforeAgent({}, { config: { configurable: { thread_id: threadId } } });
      
      const beforeModel = sessionMiddleware.beforeModel as any;
      await beforeModel({}, { config: { configurable: { thread_id: threadId } } });
      
      // Simulate tool execution
      await callbackHandler.handleToolStart({ name: "readFile", description: "Read a file from disk" }, "src/index.ts", "run-1");
    ...
    test("multiple turns in single session", async () => {
      ...
      await beforeModel1({}, { config: { configurable: { thread_id: threadId } } });
      ...
      const result1 = await afterModel1({}, { config: { configurable: { thread_id: threadId } } });
      expect(result1.acp_turnCount).toBe(1);
      
      // Turn 2
      const beforeModel2 = sessionMiddleware.beforeModel as any;
      await beforeModel2({}, { config: { configurable: { thread_id: threadId } } });
  ...
  describe("state snapshot modes", () => {
    test("initial snapshot mode emits only at start", async () => {
      ...
      const beforeAgent = sessionMiddleware.beforeAgent as any;
      const agentResult = await beforeAgent({ value: "initial" }, {
        config: { configurable: { thread_id: threadId } }
      });
      ...
      const beforeModel = sessionMiddleware.beforeModel as any;
      const modelResult = await beforeModel({ value: "model" }, {
        config: { configurable: { thread_id: threadId } }
      });
    ...
    test("final snapshot mode emits only at end", async () => {
      ...
      const afterAgent = sessionMiddleware.afterAgent as any;
      const finalResult = await afterAgent({ value: "final" }, {
        config: { configurable: { thread_id: threadId } }
      });
  ...
  describe("concurrent sessions", () => {
    test("handles multiple concurrent session lifecycles", async () => {
      ...
      await beforeAgent1({}, { config: { configurable: { thread_id: "session-1" } } });
      
      const beforeModel1 = sessionMiddleware1.beforeModel as any;
      await beforeModel1({}, { config: { configurable: { thread_id: "session-1" } } });
      
      await callbackHandler1.handleLLMStart({} as any, [], "run-1");
      await callbackHandler1.handleLLMNewToken("Session 1 response", {}, "run-1");
      await callbackHandler1.handleLLMEnd({} as any, "run-1");
      
      // Session 2 (concurrent)
      const beforeAgent2 = sessionMiddleware2.beforeAgent as any;
      await beforeAgent2({}, { config: { configurable: { thread_id: "session-2" } } });
      
      const beforeModel2 = sessionMiddleware2.beforeModel as any;
      await beforeModel2({}, { config: { configurable: { thread_id: "session-2" } } });
