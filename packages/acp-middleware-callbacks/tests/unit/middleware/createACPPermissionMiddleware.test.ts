...
} from "../../../src/middleware/createACPPermissionMiddleware";
...
describe("createACPPermissionMiddleware", () => {
	describe("initialization", () => {
		...
		test("throws error without transport", () => {
			...
			).toThrow("Permission middleware requires a transport configuration");
		});
			...
			const state = {
				messages: [
					{ _getType: () => "human", content: "Hello" },
					{
						...
						tool_calls: [{ id: "call-1", name: "unprotected_tool", args: {} }],
					},
				],
			};
			const runtime = {
	...
	describe("error handling", () => {
		test("throws when interrupt is not supported", async () => {
			...
			const state = {
				messages: [
					{ _getType: () => "human", content: "Hello" },
					{
						_getType: () => "ai",
						content: "I'll help you",
						tool_calls: [{ id: "call-1", name: "test", args: {} }],
					},
				],
			};
			const runtime = {
			...
			await expect(
				middleware.afterModel?.hook(state as any, runtime as any),
			...
			const state = {
				messages: [
					{ _getType: () => "human", content: "Hello" },
					{
						_getType: () => "ai",
						content: "I'll help you",
						tool_calls: [{ id: "call-1", name: "unknown_tool", args: {} }],
					},
				],
			};
			const runtime = {
			...
			const interruptMock = mock(async () => ({
				decisions: [
					{ type: "approve", toolCallId: "tc-1" },
					{
						type: "edit",
						toolCallId: "tc-2",
						editedAction: { name: "write_file", args: { path: "new.txt" } },
					},
	...
	describe("persistent options", () => {
		...
		test("preserves persistent option kinds correctly", async () => {
						...
						persistentOptions: [
							{
								...
								name: "Trust this operation",
								kind: "allow_always",
							},
							{
								optionId: "always_deny",
								name: "Block this operation",
								kind: "reject_always",
							},
						],
					},
			...
			const persistentOptions = callArgs.options.filter(
				(opt: any) =>
					opt.optionId === "always_allow" || opt.optionId === "always_deny",
			);
