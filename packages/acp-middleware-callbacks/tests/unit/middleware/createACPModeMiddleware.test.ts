...
import {
	...
	STANDARD_MODES,
} from "../../../src/middleware/createACPModeMiddleware";
...
describe("createACPModeMiddleware", () => {
	describe("initialization", () => {
		...
		test("throws error when defaultMode is not in modes", () => {
			expect(() => {
				...
				'Default mode "nonexistent" is not defined in modes configuration',
			);
		});
		...
		test("accepts configuration with multiple modes", () => {
			const middleware = createACPModeMiddleware({
				modes: {
					...
					readonly: {
						systemPrompt: "You can only read.",
	...
	describe("mode selection", () => {
		...
		test("throws error for undefined mode", async () => {
			...
			await expect(beforeAgent(state, runtime)).rejects.toThrow(
				'Mode "nonexistent" is not configured',
			);
		});
	});
	...
	describe("mode configuration application", () => {
		...
		test("applies allowedTools from mode config", async () => {
			...
			expect(result.acp_modeConfig.allowedTools).toEqual([
				"read_file",
				"search",
			]);
