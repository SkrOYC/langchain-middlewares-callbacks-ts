...
describe("mapToStopReason", () => {
  ...
  describe("max_tokens detection", () => {
    test("returns 'max_tokens' when llmOutput.finish_reason is 'length'", () => {
      expect(mapToStopReason({ llmOutput: { finish_reason: 'length' } })).toBe("max_tokens");
    });

    test("returns 'max_tokens' when llmOutput.finish_reason is 'context_length'", () => {
      expect(mapToStopReason({ llmOutput: { finish_reason: 'context_length' } })).toBe("max_tokens");
    });

    test("returns 'max_tokens' when llmOutput.finish_reason is 'token_limit'", () => {
      expect(mapToStopReason({ llmOutput: { finish_reason: 'token_limit' } })).toBe("max_tokens");
    });
    ...
    test("returns 'max_tokens' when state.contextLengthExceeded is true", () => {
      expect(mapToStopReason({ contextLengthExceeded: true })).toBe("max_tokens");
    });
  ...
  describe("max_turn_requests detection", () => {
    test("returns 'max_turn_requests' when turnRequests >= maxTurnRequests", () => {
      expect(mapToStopReason({ turnRequests: 10, maxTurnRequests: 10 })).toBe("max_turn_requests");
    });

    test("returns 'max_turn_requests' when state.maxStepsReached is true", () => {
      expect(mapToStopReason({ maxStepsReached: true })).toBe("max_turn_requests");
    });

    test("returns 'max_turn_requests' when state.maxTurnsReached is true", () => {
      expect(mapToStopReason({ maxTurnsReached: true })).toBe("max_turn_requests");
    });
  });

  describe("error handling", () => {
    test("returns 'end_turn' when state.error is defined", () => {
      expect(mapToStopReason({ error: "Something went wrong" })).toBe("end_turn");
    });
  ...
  describe("default case", () => {
    ...
    test("returns 'end_turn' for state with unrelated properties", () => {
      expect(mapToStopReason({ messages: [], custom: "value" })).toBe("end_turn");
    });
  });
});
...
describe("createStopReasonFromError", () => {
  describe("cancellation detection", () => {
    test("returns 'cancelled' for 'cancelled' in message", () => {
      expect(createStopReasonFromError(new Error("Operation cancelled"))).toBe("cancelled");
    });

    test("returns 'cancelled' for 'canceled' in message (US spelling)", () => {
      expect(createStopReasonFromError(new Error("Operation canceled"))).toBe("cancelled");
    });

    test("returns 'cancelled' for 'aborted' in message", () => {
      expect(createStopReasonFromError(new Error("Request aborted"))).toBe("cancelled");
    });

    test("returns 'cancelled' for 'interrupted' in message", () => {
      expect(createStopReasonFromError(new Error("Process interrupted"))).toBe("cancelled");
    });
  });

  describe("permission denial detection", () => {
    test("returns 'cancelled' for 'permission' in message", () => {
      expect(createStopReasonFromError(new Error("Permission denied"))).toBe("cancelled");
    });

    test("returns 'cancelled' for 'unauthorized' in message", () => {
      expect(createStopReasonFromError(new Error("Unauthorized access"))).toBe("cancelled");
    });

    test("returns 'cancelled' for 'forbidden' in message", () => {
      expect(createStopReasonFromError(new Error("Forbidden operation"))).toBe("cancelled");
    });
  });

  describe("refusal detection", () => {
    test("returns 'refusal' for 'refuse' in message", () => {
      expect(createStopReasonFromError(new Error("Agent refuses to continue"))).toBe("refusal");
    });

    test("returns 'refusal' for 'declined' in message", () => {
      expect(createStopReasonFromError(new Error("Request declined"))).toBe("refusal");
    });
  ...
  describe("token limit detection", () => {
    test("returns 'max_tokens' for 'token' in message", () => {
      expect(createStopReasonFromError(new Error("Token limit exceeded"))).toBe("max_tokens");
    });

    test("returns 'max_tokens' for 'length' in message", () => {
      expect(createStopReasonFromError(new Error("Token length exceeded"))).toBe("max_tokens");
    });

    test("returns 'max_tokens' for 'context' in message", () => {
      expect(createStopReasonFromError(new Error("Context length exceeded"))).toBe("max_tokens");
    });
  });

  describe("turn limit detection", () => {
    test("returns 'max_turn_requests' for 'turn' in message", () => {
      expect(createStopReasonFromError(new Error("Max turns reached"))).toBe("max_turn_requests");
    });

    test("returns 'max_turn_requests' for 'step' in message", () => {
      expect(createStopReasonFromError(new Error("Max steps exceeded"))).toBe("max_turn_requests");
    });
  });

  describe("default case", () => {
    test("returns 'end_turn' for unknown errors", () => {
      expect(createStopReasonFromError(new Error("Unknown error"))).toBe("end_turn");
    });
