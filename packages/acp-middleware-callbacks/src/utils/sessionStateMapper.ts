 ...
 */
export function validateSessionStateDetailed(state: SessionState): {
	success: boolean;
	errors?: string[];
} {
	const result = zSessionState.safeParse(state);
...
export function createSessionStateFromCheckpoint(
	...
	const checkpointValue = checkpoint.value as
		| Record<string, unknown>
		| undefined;
	const messages = checkpointValue?.messages as
		| Array<Record<string, unknown>>
		| undefined;
	const customState = checkpointValue?.custom_state as
		| Record<string, unknown>
		| undefined;
...
export function mergeSessionState(
	...
	return {
		...
		middlewareState: mergeMiddlewareState(
			current.middlewareState,
			update.middlewareState,
		),
...
export function cloneSessionState(state: SessionState): SessionState {
	...
	// @ts-expect-error: structuredClone is available in Node 17+ and modern browsers
 ...
 */
function extractMiddlewareState(
	agentState: Record<string, unknown>,
): Record<string, Record<string, unknown>> {
	if (
		agentState.middleware_state &&
		typeof agentState.middleware_state === "object"
	) {
		return agentState.middleware_state as Record<
			string,
			Record<string, unknown>
		>;
	}
