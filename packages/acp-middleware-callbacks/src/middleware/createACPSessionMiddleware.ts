...
/**
 * Creates a checkpointer-compatible store for ACP session state.
 * 
 ...
 */
export function createACPCheckpointer(sessionId: SessionId) {
  const checkpoints = new Map<string, Record<string, unknown>>();
  const sessionKey = sessionId ?? "default";
  
  return {
    get: async (threadId: string, checkpointId?: string): Promise<Record<string, unknown> | null> => {
      // If no checkpointId specified, find the latest checkpoint
      if (!checkpointId) {
        const prefix = `${sessionKey}:${threadId}:`;
        let latestKey: string | null = null;
        let latestTimestamp = -1;
        
        for (const key of checkpoints.keys()) {
          if (key.startsWith(prefix)) {
            // Extract checkpointId from key
            const ckId = key.substring(prefix.length);
            // Skip if this is already the "latest" key
            if (ckId === "latest") continue;
            
            // Simple version-based comparison (v1 < v2 < v3, etc.)
            // For numeric versions, compare as numbers
            const numMatch = ckId.match(/^v(\d+)$/);
            if (numMatch && numMatch[1]) {
              const num = parseInt(numMatch[1], 10);
              if (num > latestTimestamp) {
                latestTimestamp = num;
                latestKey = key;
              }
            }
        ...
        return latestKey ? checkpoints.get(latestKey) ?? null : null;
      }
      
      const key = `${sessionKey}:${threadId}:${checkpointId}`;
    ...
    list: async (threadId: string): Promise<Array<{ checkpointId: string }>> => {
      const result: Array<{ checkpointId: string }> = [];
      const prefix = `${sessionKey}:${threadId}:`;
      for (const key of checkpoints.keys()) {
        if (key.startsWith(prefix)) {
