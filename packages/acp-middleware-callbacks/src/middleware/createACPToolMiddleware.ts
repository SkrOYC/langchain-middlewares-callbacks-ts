...
export function createACPToolMiddleware(
	...
	return createMiddleware({
		...
		wrapToolCall: async (request, handler) => {
			...
			const toolName =
				(request as { toolCall?: { name?: string } }).toolCall?.name ??
				"unknown_tool";
			const args =
				(request as { toolCall?: { args?: unknown } }).toolCall?.args ?? {};

			const runtime = (request as { runtime?: Runtime }).runtime;
			...
			const connection = (runtime as { connection?: unknown }).connection;
			...
			if (emitToolStart) {
				const inProgressUpdate: ToolCallUpdate & {
			...
			try {
				...
				if (emitToolResults) {
					const completedUpdate: ToolCallUpdate & {
						sessionUpdate: "tool_call_update";
					} = {
						sessionUpdate: "tool_call_update",
						toolCallId,
				...
				const failedUpdate: ToolCallUpdate & {
					sessionUpdate: "tool_call_update";
				} = {
					sessionUpdate: "tool_call_update",
					toolCallId,
