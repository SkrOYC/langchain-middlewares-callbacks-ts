...
import type { Runtime } from "langchain/runtime";
import { z } from "zod";
import type {
...
export interface ACPModeMiddlewareConfig {
	 ...
	 */
	transport?: unknown; // AgentSideConnection - using unknown to avoid SDK dependency
...
export const STANDARD_MODES: Record<string, ACPModeConfig> = {
	...
	agentic: {
		systemPrompt:
			"You have full autonomy to accomplish tasks. You may use any available tools without seeking confirmation.",
	...
	interactive: {
		systemPrompt:
			"You should seek user confirmation before performing sensitive operations like file modifications, deletions, or system commands.",
	...
	readonly: {
		systemPrompt:
			"You operate in read-only mode. You can analyze and provide information but cannot modify files or execute commands. Suggest actions for the user to take instead.",
	...
	planning: {
		systemPrompt:
			"Focus on planning and analysis. Emit detailed plans for review before execution. Tool execution should be minimal and focused on gathering information needed for planning.",
...
export function createACPModeMiddleware(
	...
	if (!config.defaultMode || !(config.defaultMode in config.modes)) {
		throw new Error(
			`Default mode "${config.defaultMode}" is not defined in modes configuration`,
		);
	}
	...
	function getCurrentMode(runtimeConfig: Record<string, unknown>): string {
		...
		const configurable = runtimeConfig.configurable as
	...
	return createMiddleware({
		...
		beforeAgent: async (_state, runtime) => {
			...
			const threadId =
				...
				(agentConfig?.configurable?.thread_id as string | undefined) ??
				"default";

			const threadStateInstance = getThreadState(threadId);

			// Extract session ID
			threadStateInstance.sessionId = getSessionId(
				agentConfig,
				threadStateInstance,
			);
