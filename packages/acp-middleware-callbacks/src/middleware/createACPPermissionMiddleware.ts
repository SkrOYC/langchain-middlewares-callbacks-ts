 ...
 */
function extractToolCallsFromState(state: Record<string, unknown>):
	| Array<{
			id: string;
			name: string;
	...
	const lastMessage = [...messages]
		.reverse()
		.find((msg: any) => msg && msg._getType && msg._getType() === "ai");
...
function categorizeToolCalls(
	...
	const permissionRequired: Array<{
		name: string;
		id: string;
		args: Record<string, unknown>;
	}> = [];
	const autoApproved: Array<{
		name: string;
		id: string;
		args: Record<string, unknown>;
	}> = [];
...
function buildHITLRequest(
	...
	for (const toolCall of toolCalls) {
		...
		reviewConfigs.push({
			...
			allowedDecisions: policyConfig?.allowedResponses ?? [
				"approve",
				"edit",
				"reject",
			],
...
function processDecisions(
	...
	revisedToolCalls: Array<{
		name: string;
		id: string;
		args: Record<string, unknown>;
	}>;
	artificialMessages: Array<{
		role: string;
	...
	const revisedToolCalls: Array<{
		name: string;
		id: string;
		args: Record<string, unknown>;
	}> = [];
	const artificialMessages: Array<{
		role: string;
		content: Array<ToolCallContent>;
		tool_call_id: string;
		name: string;
...
export function createACPPermissionMiddleware(
	...
	if (
		...
		Object.keys(config.permissionPolicy).length === 0
	) {
		throw new Error(
			"Permission middleware requires a permissionPolicy configuration",
		);
	}
	...
	const descriptionPrefix =
		config.descriptionPrefix ?? "Tool execution requires approval";
	...
	async function emitToolStatus(
		...
		if (status === "pending") {
			...
			const updatePayload: ToolCallUpdate & {
	...
	return createMiddleware({
		...
		afterModel: {
			...
			hook: async (state, runtime) => {
				...
				const hitlResponse = (await runtime.interrupt(
					hitlRequest,
				)) as HITLResponse;
				...
				const hasRejections = hitlResponse.decisions.some(
					(d) => d.type === "reject",
				);
