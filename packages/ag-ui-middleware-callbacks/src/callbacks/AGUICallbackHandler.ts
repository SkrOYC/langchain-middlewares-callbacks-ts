...
import { generateDeterministicId, generateId } from "../utils/idGenerator";
import {
...
export class AGUICallbackHandler extends BaseCallbackHandler {
		...
		try {
			...
			if (
				...
				const agentRunId =
					...
					(_parentRunId
						? this.parentToAuthoritativeId.get(_parentRunId)
						: null) ||
					_parentRunId ||
					runId;
	...
	override async handleToolStart(
		...
		const agentRunId =
			(parentRunId ? this.parentToAuthoritativeId.get(parentRunId) : null) ||
			parentRunId ||
			"";
