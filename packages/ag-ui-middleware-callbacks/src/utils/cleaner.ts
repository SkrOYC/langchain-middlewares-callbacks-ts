 ...
 */
export function cleanLangChainData(data: unknown): unknown {
  ...
  if (isRecord(data)) {
    ...
    if (
      (data.lc === 1 || data.lc_serializable === true) &&
      (data.kwargs || data.lc_kwargs)
    ) {
      const kwargs = data.kwargs || data.lc_kwargs;
      return cleanLangChainData(kwargs);
    }
 ...
 */
export function extractToolOutput(output: unknown): string {
  ...
  if (isRecord(kwargs) && kwargs.content !== undefined) {
